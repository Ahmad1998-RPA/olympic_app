...
def most_successful(df, sport):
    ...
    # If a specific sport is selected, filter for that sport
    if sport != 'Overall':
        temp_df = temp_df[temp_df['Sport'] == sport]

    # Count the number of medals won by each athlete (Name)
    medal_counts = temp_df['Name'].value_counts().reset_index()
    medal_counts.columns = ['Name', 'Medals']  # Rename columns for clarity

    # Merge the medal_counts with the original DataFrame to get additional details
    merged_df = medal_counts.merge(df, left_on='Name', right_on='Name', how='left')
